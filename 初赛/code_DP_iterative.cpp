...
#else
const char readpath[] = "test_data_275w.txt"; //线上提交前：这里的路径要修改
const char writepath[] = "my_result_275w.txt";
#endif
...
void findCycle(uint v, uint layer, uint n)
{
	...
	*/
	...
	for (uint v1 : g[v].to) //第1层：v的邻居
	{
		if (v1 < cur)
			continue;
		//if (v1 == cur && endind >= 2) //找到环了:3,4元环
		//{
		//	len = endind + 1;
		...
		else if (visit[v1] != 2)
		{
			...
			trace[endind] = v1;
			visit[v1] = 2;
			for (uint v2 : g[v1].to) //第2层：v1的邻居
			{
				if (v2 < cur)
					continue;
				//if (v2 == cur && endind >= 2) //找到环了:3,4元环
				//{
				//	len = endind + 1;
				//	res[len - 3].emplace_back(trace, len);
				//}
				else if (visit[v2] != 2)
				{
					...
					trace[endind] = v2;
					visit[v2] = 2;
					for (uint v3 : g[v2].to) //第3层：v2的邻居
					{
						if (v3 < cur)
							continue;
						if (v3 == cur /*&& endind >= 2*/) //3元环
						{
							len = endind + 1;
							res[len - 3].emplace_back(trace, len);
						}
								...
								else if (visit[v4] != 2)
								{
									...
									for (uint v5 : g[v4].to) //第5层：v4的邻居
									{
										...
										else if (visit[v5] != 2)
										{
											...
											if (path2_reachable[v5])
											{
												len = 7;
												++endind;
												for (uint v6 : path_2[cur][v5])
												{
													if (visit[v6] != 2) //7元环
													{
														trace[endind] = v6;
