...
vector<uint> cur2_path2_reachable;
vector<uint> cur2_in_path2;
uint cyclesum;
		...
		for (uint vn : g[v].to)
		{
			...
			for (uint v7 : g[vn].to)
			{
				if (v7 == cur2) //cur2的6元环
				{
					...
					//res[cur2][6].push_back(Cycle(trace + 1, 6));
					//printf("cur26: trace[1] = %u, trace[6] = %u\n", trace[1], trace[6]);
					++cyclesum;
					break;
				}
			}

			if (cur2_path2_reachable[vn] == 1) //cur2的7元环
			{
				++endind;
				for (uint k : path_2[cur2][vn])
				{
					if (visit[k] != 2)
					{
						trace[endind] = k;
						res[cur2][7].emplace_back(trace + 1, 7);
						//res[cur2][7].push_back(Cycle(trace + 1, 7));
						++cyclesum;
					}
				}
				--endind;
