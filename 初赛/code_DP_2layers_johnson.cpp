...
//新增：Johnson算法改进
vector<uint> nocycle;
vector<uint> in_nocycle;
vector<uint> cur_father;
//==================================================全局变量结束========================================


bool findCycle(uint v, uint layer)
{
	/*
	v: 当前遍历到的顶点编号
	...
	*/
	...
	if (layer < 6)
	{
		...
		for (uint vn : g[v].to)
		{
			...
			{
 				if (layer + 1 >= nocycle[vn])
					continue;
				rettmp = findCycle(vn, layer + 1);
				/*if (!rettmp && !has_cur_nei)
				{
					nocycle[vn] = layer + 1;
					in_nocycle.push_back(vn);
				}*/
				ret = rettmp || ret;
			}

			else if (cur_father[vn])
			{
				ret = true;
			}
		}
	...
	{
		nocycle[v] = layer;
		in_nocycle.push_back(v);
	}
	return ret || cur_father[v];
}
...
void init()
{
	...
	nocycle.resize(nodenum, 20);
	in_nocycle.reserve(nodenum / 2);
	cur_father.resize(nodenum, 0);
}
